...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  ...
  <body>
    <header>
      <svg style="margin:  10px" width="52px" height="52px" viewBox="0 0 52 52" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        <g fill="#fff">
      ...
      <div class="main-container" >
        <aside class="sidebar" "style=margin-bottom: -50">
        <?php foreach ($categories as $key => $category): ?>
          <p><a class="sidebar" style="color-link" href="categories/{{$category->id}}">{{$category->name}}<a/></b></p>
        <?php endforeach; ?>
        </aside>
      <main class="content">
        ...
        <div>
          <div id="color-container">
            <a href="detail/{{$color->id}}" class="fill-div" style="background:{{$color->code}}">
            <div class="color-footer">{{$color->code}}</div></a>
          </div>
        </div>
        <?php endforeach; ?>
        <div>
